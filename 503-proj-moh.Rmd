...
heart_df$heart.disease<-make.names(heart_df$heart.disease)
system.time(gbm.ada.1 <- caret::train(heart.disease ~ ., 
                                      data = heart_df ,
                                      method = "rpart", 
...
heart_train$heart.disease <- as.factor(heart_train$heart.disease)
heart_test$heart.disease <- as.factor(heart_test$heart.disease)
```
##KNN
```{r}
ctrl <- trainControl(method = "cv", verboseIter = FALSE, number = 5)
knnFit <- train(heart.disease ~ ., 
...
knnPredict <- predict(knnFit,newdata = heart_test)
knn_results <- confusionMatrix(knnPredict, heart_test$heart.disease )
...
svm_fit <- train(heart.disease ~ .,data = heart_train,
                 method = "svmLinear", preProcess = c("center","scale"),
                 tuneGrid = grid_svm, trControl = ctrl)

plot(svm_fit)

svm_predict <- predict(svm_fit, newdata = heart_test)
svm_results <- confusionMatrix(svm_predict, heart_test$heart.disease)
...
rf_fit <- train(heart.disease ~ ., method = "rf", data = heart_train, ntree = 20, trControl = control,
                  tuneGrid = grid)
...
rf_results <- confusionMatrix(rf_predict, heart_test$heart.disease)
...
gbm_fit <- train(heart.disease ~ ., method = "gbm", data = heart_train,  trControl = control, verbose = FALSE, tuneGrid = gbmGrid)

plot(gbm_fit)

gbm_predict <- predict(gbm_fit, newdata = heart_test)

gbm_results <- confusionMatrix(gbm_predict,heart_test$heart.disease)
